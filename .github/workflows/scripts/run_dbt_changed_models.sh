...
dbt_run_changed_models() {
  ...
  if [ -z "$models" ]; then
    ...
    dbt run --models state:modified+1 --profile CI --target $ENV_SHORT_NAME --defer --state env-run-artifacts --favor-state --vars "{'CI_MATERIALIZATION':'view','ENV_SHORT_NAME':'$ENV_SHORT_NAME'}" --exclude tag:failing_ci
  fi
}
