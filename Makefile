install:
	...
	MICROSERVICE_PATH=orchestration VENV_NAME=data-gcp-orchestration REQUIREMENTS_NAME=orchestration-requirements.txt make install_microservice
...
upload_dags:
	$(eval COMPOSER_BUCKET_PATH=$(shell sh -c "gcloud composer environments describe data-composer-${environment} --location europe-west1 --format='value(config.dagGcsPrefix)'"))
