...
def get_dashboard_archiving_rules(row):
    ...
    if row['question_context'] == 1 and row['dashboard_context'] == 1:      # if inactivity comes from inactivity in card AND inactivity in dashboard
        return True                                                         # => ARCHIVE dashboard
    elif row['question_context'] == 1 and row['dashboard_context'] == 0:    # if inactivity comes from inactivity in card only
        return False                                                        # => DO NOT ARCHIVE dashboard
    elif row['question_context'] == 0 and row['dashboard_context'] == 0:    # if inactivity does not comes from an inactivity in card neither an inactivity in dashboard
        return False                                                        # => DO NOT ARCHIVE dashboard
    elif row['question_context'] == 0 and row['dashboard_context'] == 1:    # if inactivity comes from inactivity in dashboard only
        return True                                                         # => ARCHIVE dashboard
    else:
        return False
...
def preprocess_data_archiving(_df, object_type):
    ...
    _df = pd.concat([
        _df[_df['nb_dashboards'] != 0].query('dashboard_id.notnull()'),                   # remove rows where card are associated to dashboard but dashboard_id is null
    ...
    print(f"{_df.shape[0]} - Remove elements for which destination collection is empty...")
    ...
    _df_deduplicated = _df[[                                                              # Drop duplicates
        "card_id"
        , 'dashboard_id'
        , "dashboard_name"
        , "context"
    ...
    _df_deduplicated['question_context'] = _df_deduplicated['context'] == 'question'      # Flag if context of inactivity is question
    _df_deduplicated['dashboard_context'] = _df_deduplicated['context'] == 'dashboard'    # Flag if context of inactivity is dashboard
    
    _df_agregated = (                                                                     # Agregate context of incativity by card/dashboard pair
        _df_deduplicated
        .groupby(['card_id', 'dashboard_id', 'card_name', 'dashboard_name', 'destination_collection_id', 'nb_dashboards'], dropna=False)
        .sum()
        .reset_index()
        )
    print(f"{_df_agregated.shape[0]} - Aggregated...")
    _df_agregated['archive_dashboard'] = _df_agregated.apply(get_dashboard_archiving_rules, axis=1)    # Get archiving rule for dashboard (depending on context)
    _df_agregated['archive_question'] = _df_agregated.apply(get_question_archiving_rules, axis=1)      # Get archiving rule for question (depending on context)
    ...
    if object_type == "card":
        ...
        _df_to_archive = _df_to_archive[_df_to_archive==True].reset_index()                 # Keep cards with archiving_flag = true

        _dict_to_archive = (                                                                # Stock data in a dict
            _df_to_archive
            [['id', 'name', 'object_type','destination_collection_id']]
...
class move_to_archive:
    ...
    def move_object(self):
        ...
        if self.object_type == 'card':
            result = self.metabase_connection.update_card_collections(id_list, self.destination_collection)
