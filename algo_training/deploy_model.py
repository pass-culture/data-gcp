...
def upload_model_and_deploy_to_endpoint():
    parent_model_id = aiplatform.Model.list(
        filter=f"display_name={MODEL_NAME}", location=REGION, project=PROJECT_NAME)[0].name
    ...
    endpoint = aiplatform.Endpoint.list(
        filter=f"display_name={END_POINT_NAME}", location=REGION, project=PROJECT_NAME)[0]
