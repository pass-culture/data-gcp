...
def get_user_profile(db: Session, user_id: str) -> dict:
    ...
    user_profile = (
        db
        .query(
            enriched_user.User.user_deposit_creation_date - enriched_user.User.user_birth_date,
            ...
            func.coalesce(enriched_user.User.user_theoretical_remaining_credit, enriched_user.User.user_deposit_initial_amount)
            )
