...
def define_experimentation_sessions_query(dataset):
    return f"""
        CREATE TEMP TABLE experimentation_sessions AS (
            WITH experimentation_session AS (
                ...
                JOIN {dataset}.stock ON stock.id = booking.stockId
                JOIN {dataset}.offer ON offer.id = stock.offerId AND offer.type = 'ThingType.ACTIVATION'
...
def define_activation_dates_query(dataset):
    return f"""
        CREATE TEMP TABLE activation_dates AS (
            WITH validated_activation_booking AS (
                ...
                FROM {dataset}.booking
                JOIN {dataset}.stock ON stock.id = booking.stockId
                JOIN {dataset}.offer ON stock.offerId = offer.id AND offer.type = 'ThingType.ACTIVATION'
...
def define_date_of_first_bookings_query(dataset):
    return f"""
        CREATE TEMP TABLE date_of_first_bookings AS (
            ...
            FROM {dataset}.booking
            JOIN {dataset}.stock ON stock.id = booking.stockId
            JOIN {dataset}.offer ON offer.id = stock.offerId 
...
def define_date_of_second_bookings_query(dataset):
    return f"""
...
def define_date_of_bookings_on_third_product_query(dataset):
    return f"""
...
def define_number_of_bookings_query(dataset):
    return f"""
...
def define_number_of_non_cancelled_bookings_query(dataset):
    return f"""
...
def define_users_seniority_query(dataset):
    return f"""
...
def define_actual_amount_spent_query(dataset):
    return f"""
        CREATE TEMP TABLE actual_amount_spent AS (
            ...
            FROM {dataset}.user
            LEFT JOIN {dataset}.booking ON user.id = booking.userId 
...
def define_theoretical_amount_spent_query(dataset):
    return f"""
        CREATE TEMP TABLE theoretical_amount_spent AS (
            ...
            FROM {dataset}.user
            LEFT JOIN {dataset}.booking ON user.id = booking.userId AND booking.isCancelled IS FALSE
            WHERE user.canBookFreeOffers
            GROUP BY user.id
        );
        """
...
def define_theoretical_amount_spent_in_digital_goods_query(dataset):
    return f"""
...
def define_theoretical_amount_spent_in_physical_goods_query(dataset):
    return f"""
...
def define_theoretical_amount_spent_in_outings_query(dataset):
    return f"""
...
def define_last_booking_date_query(dataset):
    return f"""
...
def define_enriched_user_data_query(dataset):
    return f"""
...
def main(dataset):
    ...
    # Define queries
    experimentation_sessions_query = define_experimentation_sessions_query(dataset=dataset)
    activation_dates_query = define_activation_dates_query(dataset=dataset)
    date_of_first_bookings_query = define_date_of_first_bookings_query(dataset=dataset)
    date_of_second_bookings_query = define_date_of_second_bookings_query(dataset=dataset)
    date_of_bookings_on_third_product_query = define_date_of_bookings_on_third_product_query(dataset=dataset)
    number_of_bookings_query = define_number_of_bookings_query(dataset=dataset)
    number_of_non_cancelled_bookings_query = define_number_of_non_cancelled_bookings_query(dataset=dataset)
    users_seniority_query = define_users_seniority_query(dataset=dataset)
    actual_amount_spent_query = define_actual_amount_spent_query(dataset=dataset)
    theoretical_amount_spent_query = define_theoretical_amount_spent_query(dataset=dataset)
    theoretical_amount_spent_in_digital_goods_query = define_theoretical_amount_spent_in_digital_goods_query(dataset=dataset)
    theoretical_amount_spent_in_physical_goods_query = define_theoretical_amount_spent_in_physical_goods_query(dataset=dataset)
    theoretical_amount_spent_in_outings_query = define_theoretical_amount_spent_in_outings_query(dataset=dataset)
    last_booking_date_query = define_last_booking_date_query(dataset=dataset)
    humanized_id_query = define_humanized_id_query(table="user", dataset=dataset)
    enriched_user_data_query = define_enriched_user_data_query(dataset=dataset)

    overall_query = f"""
        {experimentation_sessions_query}
        {activation_dates_query}
        {date_of_first_bookings_query}
        {date_of_second_bookings_query}
        {date_of_bookings_on_third_product_query}
        {number_of_bookings_query}
        {number_of_non_cancelled_bookings_query}
        {users_seniority_query}
        {actual_amount_spent_query}
        {theoretical_amount_spent_query}
        {theoretical_amount_spent_in_digital_goods_query}
        {theoretical_amount_spent_in_physical_goods_query}
        {theoretical_amount_spent_in_outings_query}
        {last_booking_date_query}
        {humanized_id_query}
        {enriched_user_data_query}
    """
