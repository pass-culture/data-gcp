...
def restore_to_big_query():
    ...
    for table_id, uri_source in table_dict.items():
        logging.info("-------")
        logging.info("Loading table {}.".format(table_id))
        restore_table(source_path + uri_source, source_id + table_id, client, job_config)
        logging.info("-------")
...
def restore_table(uri_source, table_id, client, job_config):
    ...
    load_job = client.load_table_from_uri(
        uri_source, table_id, job_config=job_config
    ) 
