...
from http_custom.clients import SyncHttpClient, AsyncHttpClient
...
class BrevoConnector:
    ...
    def get_email_campaigns(self, status: str = "sent", limit: int = 50, offset: int = 0):
    ...
    def get_email_event_report(self, template_id: int, event: str, start_date: str, end_date: str, offset: int = 0):
...
class AsyncBrevoConnector:
    ...
    async def get_email_campaigns(self, status: str = "sent", limit: int = 50, offset: int = 0):
        url = f"{self.BASE_URL}/emailCampaigns"
        params = {"status": status, "limit": limit, "offset": offset}
        return await self.client.request("GET", url, headers=self.headers, params=params)
    ...
    async def get_email_event_report(self, template_id: int, event: str, start_date: str, end_date: str, offset: int = 0):
        url = f"{self.BASE_URL}/smtp/statistics/events"
        params = {
            "templateId": template_id,
            "event": event,
