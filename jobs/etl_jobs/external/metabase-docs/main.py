...
from core.utils import DASHBOARD_DOCS_TABLE, GCP_PROJECT_ID
...
def enrich_docs(
    ...
    try:
        ...
        if not docs:
            logger.warning(
                "No source docs in the latest notion partition — nothing to do."
            )
            return
        ...
        agent = enrich.build_agent(GCP_PROJECT_ID)

        rows, enriched, reused, failed = [], 0, 0, 0
        for doc in docs:
            pid = doc["page_id"]
            dash_id = (
                int(doc["dashboard_id"])
                if doc.get("dashboard_id") is not None
                else None
            )
