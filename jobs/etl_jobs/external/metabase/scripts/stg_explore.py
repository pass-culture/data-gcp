...
TABLES_TO_MIGRATE_PATH = (
    Path(__file__).resolve().parent.parent / "data" / "tables-to-migrate.json"
)
...
def main() -> None:
    ...
    for table_name in sorted(cache.keys()):
        ...
        native_cards = [
            cid
            for cid, info in dep.cards_using_table.items()
            if info.card_type == "native"
        ]
        qb_cards = [
            cid
            for cid, info in dep.cards_using_table.items()
            if info.card_type == "query_builder"
        ]
