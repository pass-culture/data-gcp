CREATE OR REPLACE TABLE analytics.last_6months_venue_top_offer_consultation ON CLUSTER default
ENGINE = ReplacingMergeTree()
PARTITION BY tuple()
ORDER BY (IFNULL(venue_id, 'unknown_venue_id'), rank)
SETTINGS storage_policy = 'gcs_main'
AS
WITH offer_consultations_6m AS (
    SELECT
        venue_id,
        offer_id,
