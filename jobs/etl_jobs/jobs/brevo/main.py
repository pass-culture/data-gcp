...
def save_df_to_bq(
    df: pd.DataFrame, table_id: str, schema: dict, partition_field: str = "update_date"
):
...
def save_to_historical(
    ...
    Args:
        ...
        schema (dict): Schéma BigQuery, ex: {"field": "STRING"}.
        project (str): Nom du projet GCP.
        dataset (str): Dataset BigQuery.
        table_name (str): Nom de base de la table.
        end_date (datetime): Date utilisée pour le partitionnement.
    """
    client = bigquery.Client()
...
def run(
    target: str = typer.Option(
        ..., help="Nom de la tache (newsletter ou transactional)"
    ),
    ...
    start_dt = (
        ...
        if start_date
        else (today - timedelta(days=UPDATE_WINDOW))
    )
    start_dt_newletter = datetime.strptime(
        today - timedelta(days=UPDATE_WINDOW), "%Y-%m-%d"
    )
    end_dt = datetime.strptime(end_date, "%Y-%m-%d") if end_date else today
    end_dt_newsletter = datetime.strptime(today, "%Y-%m-%d")

    logger.info(
        f"Running Brevo ETL with target={target}, audience={audience}, start={start_dt}, end={end_dt}"
    )
    ...
    if target == "newsletter":
        ...
        resp = connector.get_email_campaigns(
            start_date=start_dt_newletter, end_date=end_dt_newsletter
        )
        if not resp:
            ...
            [
                {
                    ...
                    "audience_size": c.get("statistics", {})
                    .get("globalStats", {})
                    .get("delivered"),
                    "unsubscriptions": c.get("statistics", {})
                    .get("globalStats", {})
                    .get("unsubscriptions"),
                    "open_number": c.get("statistics", {})
                    .get("globalStats", {})
                    .get("uniqueViews"),
        ...
        # table_id = (
        #     f"{GCP_PROJECT}.{BIGQUERY_RAW_DATASET}.{table_name}_histo${partition_date}"
        # )
        ...
        df_kpis = df.groupby(["tag", "template", "email", "event", "event_date"]).agg(
            {"event_date": "count"}
        )
        ...
        save_df_to_bq(
            df_pivot, table_id, transactional_histo_schema, partition_field="event_date"
        )
