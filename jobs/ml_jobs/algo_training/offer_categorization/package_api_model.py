...
from utils.constants import ENV_SHORT_NAME, MODEL_DIR
...
class ClassificationPipeline:
    ...
    def __call__(
        self, preprocessing_output: PreprocessingOutput, size: int
    ) -> PredictionOutput:
        top_labels = list(
            np.argsort(
                -self.classification_model.predict(
                    preprocessing_output.data_with_embeddings_df
                )
            )[:size]
        )
        return top_labels
...
class PreprocessingPipeline:
    ...
    def __call__(self, input_df: pd.DataFrame) -> PreprocessingOutput:
        ...
        return PreprocessingOutput(data_with_embeddings_df=input_with_embeddings_df)
    ...
    def _extract_embedding(self, input_df: pd.DataFrame) -> pd.DataFrame:
        ...
        return input_df.assign(
            name_embedding=lambda df: df["offer_name"].apply(
                lambda row: self.text_encoder.encode(row),
                axis=1,
            ),
