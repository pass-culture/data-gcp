...
def load_datasets(
    ...
    train_data[string_features] = train_data[string_features].astype(str)

    validation_data[string_features] = validation_data[string_features].astype(str)
...
def build_tf_datasets(
    ...
    validation_dataset = (
        tf.data.Dataset.from_tensor_slices(validation_features)
        .batch(batch_size=batch_size)
        .cache()
        .prefetch(tf.data.AUTOTUNE)
    )
