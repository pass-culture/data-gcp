...
matched_artists_df = pd.concat(
    [
        matched_artists_df.loc[lambda df: ~df.cluster_id.isna()],
        matched_artists_df.loc[lambda df: df.cluster_id.isna()].assign(
            artist_nickname=lambda df: df.preprocessed_artist_name
        ),
    ]
)
