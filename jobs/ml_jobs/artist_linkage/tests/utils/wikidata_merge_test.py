...
class TestMergeData:
    ...
    def test_coalesces_columns_across_overlapping_targets(self):
        """An entity matched by more than one target (e.g. gkg's broad population
        overlapping with movie/book/music's narrower ones) must keep whichever
        target's value is non-null for EACH column — not just whichever target's
        row happened to come first, discarding every other target's data for
        that entity."""
        gkg_df = pd.DataFrame(
            {
                "wikidata_id": ["Q1"],
        ...
        movie_df = pd.DataFrame(
            {
                ...
                "artist_name_fr": ["Nom Un"],
            }
        )
        merged = merge_data({"gkg": gkg_df, "movie": movie_df})

        assert len(merged) == 1
        row = merged.iloc[0]
        assert row["gkg_id"] == "/g/1234"
        assert row["img"] == "https://example.com/photo.jpg"
        assert row["artist_name_fr"] == "Nom Un"
        assert row["gkg"]
        assert row["movie"]
