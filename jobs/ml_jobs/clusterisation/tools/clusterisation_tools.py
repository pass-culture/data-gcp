...
def clusterisation_from_prebuild_encoding(
    item_full_encoding_enriched, target_nbclusters, output_table
):
    ...
    for cat in category_list:
        ...
        if not reduce_pretrained_emb:
            group_embedding_dist = (
                item_full_encoding_by_component_given_cat.values.tolist()
            )
        else:
        ...
        item_full_encoding_by_component_given_cat_super_reduced_2d = pd.DataFrame(
            X_super
        )
        ...
        dfclusters_desc["label"] = "_".join([label for label in label_list])
        dfclusters_desc.to_gbq(
            f"clean_{ENV_SHORT_NAME}.{output_table}_{target_nbclusters}_{cat}",
            if_exists="replace",
        )

    print("Input for graph ready and saved!")
    return
