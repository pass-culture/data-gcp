...
def partition_parquet(
    ...
    output_base = f"gs://mlflow-bucket-{gcp_env}/streamlit_data/chatbot_edito/offers_{env}/partitioned"

    logger.info(f"Starting partitioning for environment: {env} ({gcp_env})")
    logger.info(f"Source parquet file pattern: {parquet_file}")
    logger.info(f"Output base path for partitioned data: {output_base}")
    if dry_run:
    ...
    try:
        ...
        logger.info(
            f'1. Update app/constants.py: PARQUET_FILE = "{output_base}/**/*.parquet"'
        )
