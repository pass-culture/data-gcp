...
with col2:
    ...
    if st.button("🔥 Warmup API", use_container_width=True, type="secondary"):
        try:
            warmup_response = requests.get(f"{base_url}/warmup", timeout=300)
...
if use_filters:
    ...
    with col1:
        if st.button("➕ Add Filter", use_container_width=True, type="primary"):  # noqa: RUF001
            st.session_state.filters.append(
                {"column": AVAILABLE_COLUMNS[0], "operator": "=", "value": ""}
            )
            st.rerun()
    ...
    with col3:
        if (len(st.session_state.filters) > 0) and st.button(
            ...
            with st.container():
                ...
                with col1:
                    filter_item["column"] = st.selectbox(
                        ...
                        key=f"column_{i}",
                        index=AVAILABLE_COLUMNS.index(filter_item["column"])
                        if filter_item["column"] in AVAILABLE_COLUMNS
                        else 0,
                ...
                with col2:
                    filter_item["operator"] = st.selectbox(
                        ...
                        key=f"operator_{i}",
                        index=AVAILABLE_OPERATORS.index(filter_item["operator"])
                        if filter_item["operator"] in AVAILABLE_OPERATORS
                        else 0,
                        label_visibility="collapsed" if i > 0 else "visible",
                    )
                ...
                with col3:
                    ...
                    if filter_item["operator"] in ["in", "not in"]:
                        value_input = st.text_input(
                            ...
                            value=",".join(map(str, filter_item["value"]))
                            if isinstance(filter_item["value"], list)
                            else str(filter_item["value"]),
                            key=f"value_{i}",
                            help="Enter values separated by commas (e.g., 1,2,3)",
...
# ============= PAYLOAD PREVIEW =============
# Build payload
payload = {"instances": [{"search_query": search_query}]}
...
with col2:
    send_button = st.button(
        "🚀 Send Predict Request", use_container_width=True, type="primary"
    )
        ...
        try:
            ...
            if response.ok:
                ...
                with col3:
                    ...
                    st.metric(
                        "Offers Found", len(offers) if isinstance(offers, list) else 0
                    )
                ...
                if offers and isinstance(offers, list):
                    st.subheader("🎯 Offers")
                    st.dataframe(
                        pd.DataFrame(offers).head(100), use_container_width=True
                    )
                else:
