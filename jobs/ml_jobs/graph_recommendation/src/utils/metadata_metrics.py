...
def get_gtl_walk_score(query_gtl_id: str, result_gtl_id: str) -> float:
    ...
    dist = _get_gtl_walk_dist(_strip_gtl_prefix(query_gtl_id), _strip_gtl_prefix(result_gtl_id))
    if dist == np.inf:
        return 0.0
    return 1 / (1 + dist)


def get_gtl_retrieval_score(query_gtl_id: str | None, result_gtl_id: str | None) -> float | None:
    """
    ...
    result_prefix = _extract_prefix(result_gtl_id) if result_gtl_id is not None else None
