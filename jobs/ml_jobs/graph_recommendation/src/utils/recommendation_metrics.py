...
def _compute_metrics_at_k(
    ...
    users_with_positive_scores = rating_true[rating_true["rating"] > 0][
        "userID"
    ].unique()
    if len(users_with_positive_scores) == 0:
        logger.warning(
            f"No positive scores found for score column"
            f" '{score_column}' at k={k}. Skipping."
        )
        return {
            "score_column": score_column,
            "k": k,
    ...
    rating_true_filtered = rating_true[
        rating_true["userID"].isin(users_with_positive_scores)
    ]
    rating_pred_filtered = rating_pred_sorted[
        rating_pred_sorted["userID"].isin(users_with_positive_scores)
    ]
