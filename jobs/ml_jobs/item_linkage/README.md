...
> **In one sentence:** for every catalog offer, find the ones that are really the *item* and give them one shared `item_id`, anchoring to a real product when one exists, otherwise minting a synthetic cluster.
...
```mermaid
flowchart TD
    ingest["1 · Ingest sources & candidates (import_data · BigQuery SQL)<br/>SOURCES — offer_product_id IS NOT NULL → product-* (trusted catalog)<br/>CANDIDATES — offer_product_id IS NULL → offer-* (offers to attach)"]
    ingest -->|export → GCS · start GCE| pre["2 · Preprocess (preprocess.py)<br/>normalize text; offer_name → oeuvre (work) + edition (tome/vol/№)<br/>embeddings 128-dim, L2-normalized"]
    pre -->|split by subcategory & product presence| prod0
    pre --> off0

    subgraph P [Product workflow → product-*]
        direction TB
        prod0["build_semantic_space.py<br/>index product sources · LanceDB IVF_PQ, cosine"]
        prod0 --> prod1["linkage_candidates.py<br/>retrieve top-5 · offer → nearest products"]
        prod1 --> prod2["link_items.py<br/>verify title · Jaro-Winkler ≥ 0.90"]
        prod2 --> prod3["→ linked_product<br/>offer adopts the product's item_id_synchro"]
    end

    subgraph O [Offer workflow → item_cluster_*]
        direction TB
        off0["build_semantic_space.py<br/>index offers · non-synchro + product leftovers"]
        off0 --> off1["linkage_candidates.py<br/>retrieve top-5 · offer ↔ offer"]
        off1 --> off2["link_items.py<br/>verify title · Jaro-Winkler ≥ 0.95"]
        off2 --> off3["assign_linked_ids.py<br/>graph → connected components → item_cluster_N"]
    end

    prod3 --> unify["3 · Unify → item_offer_mapping (build_mapping.sql)<br/>UNION of both tables, normalized to (item_id, offer_id)"]
    off3 --> unify
    unify -->|consumed by dbt| down["4 · Downstream: offers adopt the item_id<br/>int_applicative__offer_item_id · mrt_global__offer"]
    down -.->|⟲ feedback loop — see gotcha below| ingest
```

### The core idea — three moves: recall, then precision, then identity

1. **Recall — vector retrieval.** ANN search narrows the whole catalog to a few plausible neighbors, hard-filtered to the same `edition` + `subcategory`, and bounded by a cosine-distance ceiling (`SEMANTIC_RETRIEVAL_UPPER_BOUND`). It only shortlists.
2. **Precision — fuzzy verification.** Jaro-Winkler on the cleaned title (`oeuvre`) decides the link. This is the real gate a neighbor is only linked if titles are near-identical (similarity at least 0.9).
3. **Identity — graph clustering.** Pairwise links become graph edges; connected components give transitive groups a single shared id (offers only).
...
|---|---|---|---|
| `product-*` | The offer is attached to a catalog product. | Free, from the product catalog — all offers of one product share it. | **Yes** — external id |
