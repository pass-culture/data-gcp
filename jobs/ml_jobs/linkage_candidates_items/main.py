...
def download_model(experiment_name, run_id, local_retrieval_path):
    ...
    container_id = subprocess.run(
        "docker ps -lq", shell=True, capture_output=True, text=True
    ).stdout.strip()

    # Copy files from the Docker container
    subprocess.run(
        f"docker cp {container_id}:/app/ {local_retrieval_path}", shell=True, check=True
    )
...
def generate_semanctic_candidates(
    ...
    for index, row in data_ready.iterrows():
        ...
        params = {}
        result_df = get_semantic_candidates(
            model_retrieval, row.vector, params, "cosine", k=20
        )
        if len(result_df) == 0:
...
@app.command()
def main(
    source_file_path: str = typer.Option("./data.parquet", help="Input table path"),
    output_file_path: str = typer.Option(
        "./linkage_candidates.parquet", help="Output table path "
    ),
