...
The API accepts a single instance in `instances[0]`, validates it with `PredictionRequest`, then dispatches to the right handler based on `model_type` and the loaded embedding model.

```mermaid
flowchart TD
  A[/POST /predict/]:::entry
  B[Parse JSON body]
  C{instances list present?}
  D[Validate PredictionRequest]
  E{items empty and offer_id present?}
  F[Map offer_id to items]
  G{model_type}
  H[RecommendationHandler]
  I["SimilarOfferHandler<br/>two_tower"]
  J["SimilarOfferHandler<br/>semantic"]
  K[TextSearchHandler]
  L[SearchByTopsHandler]
  M{user_id present?}
  N[Load user vector]
  O{user vector found?}
  P[search_by_vector]
  Q{predictions found?}
  R[Return empty PredictionResult]
  S{items present?}
  T[Load item vectors]
  U{item vector found?}
  V[Skip item and continue]
  W[search_by_vector per item]
  X{any predictions found?}
  Y[Aggregate multi-item results if needed]
  Z[search_by_text]
  AA[search_by_tops]
  AB[Return PredictionResult]
  ...
  X -- no --> R
  X -- yes --> Y --> AB
  G -- semantic_search --> J --> S
  G -- text_search --> K --> Z --> AB
  G -- filter/tops --> L --> AA --> AB

  classDef entry fill:#cce5ff,stroke:#004085,color:#004085,stroke-width:2px;
  class A entry;
```
