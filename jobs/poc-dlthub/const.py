...
PYARROW_TABLE_SCHEMA = pa.schema(
    [
        ...
        pa.field("user_full_address", pa.string(), nullable=True),
        pa.field("result_status", pa.string(), nullable=False),
        pa.field("longitude", pa.string(), nullable=True),
        ...
        pa.field("result_departmentcode", pa.string(), nullable=True),
        pa.field("result_municipalitycode", pa.string(), nullable=True),
        pa.field("result_section", pa.string(), nullable=True),
        pa.field("result_sheet", pa.string(), nullable=True),
        pa.field("result_number", pa.string(), nullable=True),
        pa.field("result_oldmunicipalitycode", pa.string(), nullable=True),
        pa.field("result_districtcode", pa.string(), nullable=True),
    ]
)

QUERY = """with profile_completion_check as (select user_id,
               timestamp (datecreated) as user_last_modified_at
              , json_extract(result_content
              , '$.city') as user_city
              , json_extract(result_content
              , '$.address') as user_address
              , json_extract(result_content
              , '$.postal_code') as user_postal_code
           from `passculture-data-prod`.`raw_prod`.`applicative_database_beneficiary_fraud_check`
           where type = 'PROFILE_COMPLETION'
             and status = 'OK'
               )
               , profile_completion as (
           select
               pcc.user_last_modified_at, pcc.user_id, trim (
               concat(
               replace(replace(pcc.user_address, '\\r', ''), '\\n', ''), ' ', pcc.user_postal_code, ' ', pcc.user_city
               )
           ...
           select
               adu.user_id, timestamp (adu.user_creation_date) as user_last_modified_at, trim (
               concat(
               replace(replace(adu.user_address, '\\r', ''), '\\n', ''), ' ', adu.user_postal_code, ' ', adu.user_city
               )
               ) as user_full_address
           from `passculture-data-prod`.`raw_prod`.`applicative_database_user` as adu
           where
           -- beneficiary update should have all fields
           ...
           from `passculture-data-prod`.`raw_prod`.`user_address`
           group by user_id
               )
...
from user_candidates as uc
         ...
         inner join
     `passculture-data-prod`.`raw_prod`.`applicative_database_user` as u on uc.user_id = u.user_id
-- where
