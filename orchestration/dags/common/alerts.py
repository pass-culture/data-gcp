...
def dbt_test_fail_slack_alert(test_output_dict,fail_type,dag,job_type=ENV_SHORT_NAME, **context):
    webhook_token = JOB_TYPE.get(job_type)
    ...
    for node, failed_tests in test_output_dict.items():
        if len(failed_tests)>0:
            slack_msg = '\n'.join([slack_msg,f"""*model* <{node}>"""])+' has failed following tests with message:\n'
            for testdict in failed_tests:
                if len(testdict)>0:
                    for test,message in testdict.items():
                        slack_msg = '\n\t'.join([slack_msg,f"""*Test* <{test}>"""+'\n\t'+f"""{message}>"""])
