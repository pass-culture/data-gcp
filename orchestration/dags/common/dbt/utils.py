...
def build_simplified_manifest(json_dict_data):
    # build a simplified manifest for data_gcp_dbt project's models
    simplified_manifest = {
        node: {
            "redirect_dep": None,
            "model_node": node,
    ...
    for node in tests_nodes:
        ...
        # test_alias = json_dict_data["nodes"][node]['name']#node.split(".")[-2]
        test_alias = json_dict_data["nodes"][node]['name']
        test_config = json_dict_data["nodes"][node]["config"].get("severity", None)
        try:
             test_config = test_config.lower()
        except AttributeError:
            pass
        parents = json_dict_data['parent_map'][node]
        attached_parent = json_dict_data["nodes"][node]['attached_node']
                ...
                simplified_manifest[p_node]["model_tests"][test_config] = [
                        {
                            ...
                            "attached": True if p_node == attached_parent else False,
                        }
                    ]
            else:
                simplified_manifest[p_node]["model_tests"][test_config] += [
    ...
    simplified_manifest = { **simplified_manifest, 
                           **{node: {"test_alias":json_dict_data["nodes"][node]['name'],
                                     "resource_type":"test",
                                     "attached_parent":json_dict_data["nodes"][node]['attached_node'],
                                     "parents":json_dict_data['parent_map'][node],
                                     } 
