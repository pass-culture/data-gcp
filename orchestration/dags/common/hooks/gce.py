...
class DeferrableSSHGCEJobManager(SSHGCEJobManager):
    ...
    def _parse_check_command_output(self, raw: str | bytes) -> DeferrableSSHJobStatus:
        """
        Extracts STATUS, STDOUT, STDERR, PID, and raw logs from SSH command output.

        Returns:
            DeferrableSSHJobStatus: An object containing:
                - status (str | None): Job status ("running", "completed", "failed", etc.)
                - stdout (str): Captured standard output from the remote job
                - stderr (str): Captured standard error from the remote job
                - pid (str | None): Process ID of the running or completed job
                - logs (str): Full raw SSH output for debugging
        """
