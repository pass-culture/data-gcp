...
{% docs column__venue_siret %} SIRET number of the venue. A venue can have no SIRET (especially when it hosts public but does not belong to the offerer) {% enddocs %}
{% docs column__venue_is_virtual %} Indicates if the venue is virtual. Each offerer has a virtual venue, it is only used to publish digital offers. NB : digital venues will soon be deleted, their offers/bookings/stocks will be transfer to other venues of the offerer. {% enddocs %}
...
{% docs column__venue_type_label %} Type of the venue ('Musée', 'Cinéma','Librairie', etc). Selected by the partner in a drop-down list during subscription. If you need to study bookings and offers per cultural sector, use the offer categories instead. NB : this segmentation is limited and currently being revised {% enddocs %}
