WITH booking_ranking_in_category_view AS (
    ...
    FROM
        ...
        INNER JOIN {{ ref('stock')}} AS offer ON stock.offer_id = offer.offer_id
    ORDER BY
        booking.booking_id
)
...
FROM
    {{ ref('booking')}} AS booking
    INNER JOIN {{ ref('stock')}} AS stock ON booking.stock_id = stock.stock_id
    LEFT JOIN {{ ref('stock')}} AS offer ON offer.offer_id = stock.offer_id
    ...
    INNER JOIN {{ ref('subcategories')}} subcategories ON offer.offer_subcategoryId = subcategories.id
