WITH ranked_deposit AS (
    ...
    FROM
        {{ source('raw','applicative_database_educational_deposit') }} AS educational_deposit
        JOIN {{ source('raw','applicative_database_educational_year') }} AS educational_year ON educational_deposit.educational_year_id = educational_year.adage_id
),
...
last_booking AS (
    ...
    FROM
        bookings_infos
        JOIN {{ source('raw','applicative_database_collective_stock') }} AS collective_stock ON bookings_infos.collective_stock_id = collective_stock.collective_stock_id
...
students_educonnectes AS (
    ...
    FROM {{ source('raw','applicative_database_beneficiary_fraud_check') }} AS beneficiary_fraud_check
...
FROM
    ...
    LEFT JOIN {{ source('raw','rural_city_type_data') }} as rurality
