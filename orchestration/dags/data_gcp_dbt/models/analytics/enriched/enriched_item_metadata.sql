WITH offer_booking_information_view AS (
    ...
    FROM
        {{ ref('offer') }} AS offer
        LEFT JOIN {{ ref('stock') }} AS stock ON stock.offer_id = offer.offer_id
...
simple_item_clusters AS (
    SELECT 
        ic.item_id,  
        ANY_VALUE(ic.semantic_cluster_id) as simple_cluster_id,
        ANY_VALUE(it.topic_id) as simple_topic_id
    FROM {{ source('clean','simple_item_clusters') }} ic
    LEFT JOIN {{ source('clean','simple_item_topics') }} it on it.item_id = ic.item_id
    GROUP BY 1
),
unconstained_item_clusters AS (
    SELECT 
        ic.item_id,  
        ANY_VALUE(ic.semantic_cluster_id) as unconstrained_cluster_id,
        ANY_VALUE(it.topic_id) as unconstrained_topic_id
    FROM {{ source('clean','unconstrained_item_clusters') }} ic
    LEFT JOIN {{ source('clean','unconstrained_item_topics') }} it on it.item_id = ic.item_id
    GROUP BY 1
),
...
enriched_items AS (
    SELECT 
        ...
        ic.cluster_id,
        sic.simple_topic_id,
        sic.simple_cluster_id,
        uic.unconstrained_topic_id,
        uic.unconstrained_cluster_id,
    ...
    LEFT JOIN unconstained_item_clusters uic on uic.item_id = offer_ids.item_id
)
