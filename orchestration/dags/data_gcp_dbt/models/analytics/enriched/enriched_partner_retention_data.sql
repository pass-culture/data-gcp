...
LEFT JOIN {{ref ('enriched_offer_data')}} AS enriched_offer_data ON enriched_cultural_partner_data.partner_id = enriched_offer_data.partner_id
GROUP BY 1,2,3)
...
FROM {{ ref('enriched_cultural_partner_data')}} AS enriched_cultural_partner_data
JOIN {{ ref ('partner_type_bookability_frequency')}} AS partner_type_bookability_frequency USING(partner_type)
LEFT JOIN {{ ref('enriched_booking_data')}} AS enriched_booking_data ON enriched_cultural_partner_data.partner_id = enriched_booking_data.partner_id
...
FROM {{ ref('enriched_cultural_partner_data')}} AS enriched_cultural_partner_data
JOIN {{ ref ('partner_type_bookability_frequency')}} AS partner_type_bookability_frequency USING(partner_type)
LEFT JOIN {{ ref('enriched_collective_offer_data')}} AS enriched_collective_offer_data ON enriched_cultural_partner_data.partner_id = enriched_collective_offer_data.partner_id
GROUP BY 1,2,3)
...
FROM {{ ref('enriched_collective_booking_data')}} AS enriched_collective_booking_data
LEFT JOIN {{ ref('enriched_venue_data')}} AS enriched_venue_data USING(venue_id)
...
LEFT JOIN {{ ref('enriched_offer_data')}} AS enriched_offer_data ON enriched_venue_data.venue_id = enriched_offer_data.venue_id
LEFT JOIN {{ ref('favorite')}} AS favorite ON enriched_offer_data.offer_id = favorite.offerId)
...
FROM favorites1
JOIN {{ ref('enriched_cultural_partner_data')}} AS enriched_cultural_partner_data USING(partner_id)
JOIN {{ ref ('partner_type_bookability_frequency')}} AS partner_type_bookability_frequency ON enriched_cultural_partner_data.partner_type = partner_type_bookability_frequency.partner_type
GROUP BY 1,2,3)
...
LEFT JOIN {{ ref('enriched_venue_data')}} AS venue on consult.venue_id = venue.venue_id
LEFT JOIN {{ ref('enriched_cultural_partner_data')}} as enriched_cultural_partner_data on (CASE WHEN venue.venue_is_permanent THEN CONCAT("venue-",venue.venue_id) ELSE CONCAT("offerer-", venue_managing_offerer_id) END) = enriched_cultural_partner_data.partner_id
GROUP BY 1
)
...
FROM {{ ref('enriched_venue_data')}} AS enriched_venue_data
LEFT JOIN {{ ref('enriched_offerer_data')}} AS enriched_offerer_data on enriched_venue_data.venue_managing_offerer_id = enriched_offerer_data.offerer_id
)
...
FROM {{ ref('enriched_venue_data')}} AS enriched_venue_data
JOIN {{ ref('enriched_offerer_data')}} AS enriched_offerer_data ON enriched_venue_data.venue_managing_offerer_id = enriched_offerer_data.offerer_id
LEFT JOIN {{ ref('siren_data')}} AS siren_data ON enriched_offerer_data.offerer_siren = siren_data.siren )
...
FROM {{ ref('enriched_venue_data')}} AS enriched_venue_data
LEFT JOIN {{ ref('offer')}}  AS offer ON offer.venue_id = enriched_venue_data.venue_id
...
FROM {{ ref('enriched_venue_data')}} AS enriched_venue_data
LEFT JOIN {{ ref('enriched_venue_provider_data')}} AS enriched_venue_provider_data ON enriched_venue_provider_data.venue_id = enriched_venue_data.venue_id AND is_active )
...
FROM {{ ref('enriched_venue_data')}} AS enriched_venue_data
LEFT JOIN {{ ref('venue_reimbursement_point_link')}} AS venue_reimbursement_point_link ON enriched_venue_data.venue_id = venue_reimbursement_point_link.venue_id)
...
LEFT JOIN {{ ref('enriched_venue_data')}} AS enriched_venue_data on bookable_venue_history.venue_id = enriched_venue_data.venue_id
