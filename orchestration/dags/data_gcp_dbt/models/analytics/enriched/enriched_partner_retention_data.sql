...
LEFT JOIN {{ ref('enriched_venue_data')}} USING(venue_id)
...
LEFT JOIN {{ ref('enriched_offer_data')}} ON enriched_venue_data.venue_id = enriched_offer_data.venue_id
...
LEFT JOIN {{ ref('enriched_venue_data')}} on consult.venue_id = venue.venue_id
...
FROM {{ ref('enriched_venue_data')}}
LEFT JOIN {{ ref('venue_reimbursement_point_link')}} AS venue_reimbursement_point_link ON enriched_venue_data.venue_id = applicative_database_venue_reimbursement_point_link.venue_id)
