WITH clean_payment_status AS (
    ...
    FROM
        {{ source('raw', 'applicative_database_payment_status') }}
    GROUP BY
        paymentId
),
...
CASE
    ...
    FROM {{ ref('enriched_booking_data') }} AS booking
        LEFT JOIN {{ source('raw', 'applicative_database_pricing') }} AS pricing ON booking.booking_id = pricing.bookingId
        LEFT JOIN {{ source('raw', 'applicative_database_pricing_line') }} AS pricing_line ON pricing.id = pricing_line.pricingId
),
...
individuel AS (
    ...
    FROM
        ...
        LEFT JOIN {{ source('raw', 'applicative_database_stock') }} AS applicative_database_stock ON booking.stock_id = applicative_database_stock.stock_id
        LEFT JOIN {{ source('raw', 'applicative_database_deposit') }} AS applicative_database_deposit ON booking.deposit_id = applicative_database_deposit.id
        LEFT JOIN {{ source('raw', 'applicative_database_pricing') }} AS pricing ON booking.booking_id = pricing.bookingId
        ...
        LEFT JOIN {{ source('raw', 'applicative_database_cashflow') }} AS  cashflow ON cashflow_pricing.cashflowId = cashflow.id
        LEFT JOIN {{ source('raw', 'applicative_database_cashflow_batch') }} AS cashflow_batch ON cashflow.batchId = cashflow_batch.id
        LEFT JOIN {{ source('raw', 'applicative_database_invoice_cashflow') }} AS invoice_cashflow ON cashflow.id = invoice_cashflow.cashflow_id
        LEFT JOIN {{ source('raw', 'applicative_database_invoice') }} AS invoice ON invoice_cashflow.invoice_id = invoice.invoice_id
        LEFT JOIN {{ source('raw', 'applicative_database_invoice_line') }} AS invoice_line ON invoice.invoice_id = invoice_line.invoice_id
    WHERE
...
CASE
            WHEN category = 'offerer contribution' THEN pricing_line.amount
        END AS offerer_contribution
    FROM {{ ref('enriched_collective_booking_data') }} AS collective_booking
