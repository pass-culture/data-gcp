...
from {{ ref('firebase_visits') }} firebase_visits
    inner join {{ ref('firebase_session_origin') }} firebase_session_origin
        on
            firebase_session_origin.unique_session_id = firebase_visits.unique_session_id
            and firebase_session_origin.traffic_campaign is not NULL
    left join {{ ref('aggregated_daily_user_used_activity') }} daily_activity
        on
            daily_activity.user_id = firebase_visits.user_id
