...
WITH mapping_module_name_and_id AS (
    ...
    FROM
        (
            ...
            from
                {{ ref("int_contentful__relationships") }} relations
                inner join {{ ref("int_contentful__entries") }} entries on entries.id = relations.child
            WHERE
...
firebase_module_events AS (
    ...
    FROM
        firebase_events e
        LEFT JOIN  {{ ref("int_contentful__entries") }} entries on entries.id = e.module_id
),
