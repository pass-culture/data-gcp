...
with
        ...
        from
            (
                select
                    ...
                    row_number() over (
                        partition by entries.title order by updated_at desc
                    ) as rnk
                from {{ ref("int_contentful__relationship") }} relations
                inner join
                    {{ ref("int_contentful__entry") }} entries
                    on entries.id = relations.child
                where
    ...
    firebase_events as (
        select
            ...
            e.* except (module_id, module_name)
        from {{ ref("int_firebase__native_event") }} e
        left join
            mapping_module_name_and_id c_name on c_name.module_name = e.module_name
        where
            event_name in (
                'ConsultOffer',
                "ConsultVenue",
            ...
            and (origin = 'home' or origin = 'exclusivity' or origin is null)
    ...
    firebase_module_events as (
        select
            ...
            e.booking_id,
            case
                when entries.content_type = "recommendation"
                then e.reco_call_id
                else null
            end as call_id,
            -- modules
            ...
            coalesce(
                ...
                last_value(module_index ignore nulls) over (
                    partition by user_id, session_id, module_id
                    order by event_timestamp
                    range between unbounded preceding and current row
                ),
    ...
    firebase_conversion_step as (
        select
            ...
            row_number() over (
                partition by
                    conv.session_id, conv.user_id, conv.offer_id, conv.event_name
                order by event.event_timestamp desc
            ) as rank
        from {{ ref("int_firebase__native_event") }} conv
        inner join
            firebase_module_events event
            on event.unique_session_id = conv.unique_session_id
            and event.offer_id = conv.offer_id
            and event.user_id = conv.user_id  -- conversion event after click event
        where
            conv.event_name in ('BookingConfirmation', 'HasAddedOfferToFavorites')
...
select
    ...
    e.event_name,
    case
        when event_name = "ModuleDisplayedOnHomePage"
        then "display"
        when
            event_name in (
                "ConsultOffer",
        ...
        then "click"
        when event_name in ("SeeMoreClicked")
        then "see_more_click"
        when event_name = "HasAddedOfferToFavorites"
        then "favorite"
        when event_name = "BookingConfirmation"
        then "booking"
    end as event_type,
    e.offer_id,
    e.booking_id,
