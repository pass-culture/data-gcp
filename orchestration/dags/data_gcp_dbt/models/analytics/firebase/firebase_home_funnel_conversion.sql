...
clicked as (
    ...
    where
        event_name in ("ExclusivityBlockClicked", "CategoryBlockClicked", "HighlightBlockClicked", "BusinessBlockClicked", "ConsultVideo")
...
consultations as (
    ...
    from venue
        full outer join offer
            on
                offer.unique_session_id = venue.unique_session_id
        ...
        join relationships -- inner join to get only known relationships between playlist and homepages.
            on
                relationships.playlist_id = coalesce(offer.module_id, venue.module_id)
...
from displayed
    left join clicked
        on
            displayed.unique_session_id = clicked.unique_session_id
    ...
    left join {{ ref('firebase_bookings') }} as bookings
        on
            displayed.unique_session_id = bookings.unique_session_id
