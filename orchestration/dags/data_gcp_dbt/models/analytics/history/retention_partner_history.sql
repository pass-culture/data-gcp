with
    ...
    scheduled_offers as (
        ...
        where
            ...
            and date_add(date('2024-04-30'), interval offset day) >= offer_creation_date  -- Les jours depuis la création de l'offre
            and date_add(date('2024-04-30'), interval offset day)
            <= offer_publication_date  -- Les jours avant la publication de l'offre
        group by 1, 2
    ),
...
select
    ...
    max(
        case
            when total_bookable_offers != 0 or individual_scheduled_offers != 0 then day
        end
    ) over (
        partition by partner_id
        order by day
    ...
    max(
        case
            when individual_bookable_offers != 0 or individual_scheduled_offers != 0
            then day
        end
    ) over (
        partition by partner_id
    ...
    date_diff(
        ...
        max(
            case
                when total_bookable_offers != 0 or individual_scheduled_offers != 0
                then day
            end
        ) over (
            partition by partner_id
