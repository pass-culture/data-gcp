with
    consultation as (
        select
            *,
            lag(consulted_items) over (
                partition by user_id order by consultation_date
            ) as last_consulted_items,
            lag(consulted_origins) over (
                partition by user_id order by consultation_date
            ) as last_consulted_origins
    ...
    new_consultation as (
        select
            ...
            array(
                select *
                from consultation.consulted_items
                except distinct
                (select * from consultation.last_consulted_items)
            ) as new_consulted_items,
            array(
                select *
                from consultation.consulted_origins
                except distinct
    ...
    new_consultation_item as (
        select
            ...
            {% for feature in discovery_vars("discovery_features") %}
                case
                    when
                        consultation_date = min(consultation_date) over (
        ...
        left join
            {{ ref("ml_input__item_metadata") }} metadata
    ...
    new_consultation_origin as (
        select
            ...
            case
                when
                    consultation_date
                    = min(consultation_date) over (partition by user_id, new_origins)
                then 1
                else 0
    ...
    agg_new_consultation_origin as (
        select
            user_id, consultation_date, sum(new_origins_discovery) as discovery_origin
...
select
    coalesce(
        agg_new_consultation_item.user_id, agg_new_consultation_origin.user_id
    ) as user_id,
    coalesce(
        agg_new_consultation_item.consultation_date,
        agg_new_consultation_origin.consultation_date
    ...
    {% endfor %},
    {% for feature in discovery_vars("discovery_features") %}
        coalesce(discovery_{{ feature }}, 0) {% if not loop.last -%} + {%- endif %}
    {% endfor %}
    + coalesce(discovery_origin, 0) as discovery_delta
from agg_new_consultation_item
full outer join agg_new_consultation_origin using (user_id, consultation_date)
