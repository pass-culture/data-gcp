...
with offerer_offer_info as (
    ...
    where
        o.collective_offer_creation_date >= DATE_SUB(CURRENT_DATE(), interval 2 month) -- uniquement sur les 2 derniers mois
        and offer_is_template is TRUE
    qualify ROW_NUMBER() over (partition by offerer_id order by collective_offer_creation_date desc) = 1
