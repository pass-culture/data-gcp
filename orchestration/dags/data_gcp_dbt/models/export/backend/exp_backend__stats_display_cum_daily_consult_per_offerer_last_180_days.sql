...
with
    all_days_since_activation as (
        select distinct
            ...
            mrt_global__offerer.offerer_creation_date,
            date_add(
                date(mrt_global__offerer.offerer_creation_date), interval day_offset day
            ) as day
        from {{ ref("mrt_global__offerer") }} as mrt_global__offerer
        inner join {{ ref("mrt_native__daily_offer_consultation") }} using (offerer_id)
        cross join
            unnest(
                generate_array(0, date_diff(current_date(), '2018-01-01', day))
            ) as day_offset
        where
            date_add(
                date(mrt_global__offerer.offerer_creation_date), interval day_offset day
            )
            >= mrt_global__offerer.offerer_creation_date  -- Les jours depuis la création de la structure
            and date_add(
                date(mrt_global__offerer.offerer_creation_date), interval day_offset day
            )
...
select
    all_days_since_activation.day as event_date,
    all_days_since_activation.offerer_id,
    date_diff(current_date, all_days_since_activation.day, day) as day_seniority,
    coalesce(cum_consult_per_day.nb_daily_consult, 0) as nb_daily_consult,
    max(cum_consult_per_day.cum_consult) over (
        partition by all_days_since_activation.offerer_id
        order by all_days_since_activation.day
...
where date_diff(current_date, all_days_since_activation.day, day) <= 180
