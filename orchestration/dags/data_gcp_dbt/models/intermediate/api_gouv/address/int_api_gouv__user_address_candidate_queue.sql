with profile_completion_check as (
  select
      user_id,
...
user_candidates as (
  select
    ...
    user_full_address
  from all_users
  qualify row_number() over( partition by user_id order by user_last_modified_at desc ) = 1
),
...
inner join {{ source("raw", "applicative_database_user") }} as u on uc.user_id = u.user_id
where
