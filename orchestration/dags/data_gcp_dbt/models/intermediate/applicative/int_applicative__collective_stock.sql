...
select
    ...
    case when total_non_cancelled_collective_bookings is null
        and (DATE(cs.collective_stock_booking_limit_date_time) > (DATE("{{ ds() }}") - 1) or cs.collective_stock_booking_limit_date_time is NULL)
        and (DATE(cs.collective_stock_beginning_date_time) > (DATE("{{ ds() }}") -1) or cs.collective_stock_beginning_date_time is NULL) then true else false end as collective_stock_is_bookable
