...
select
    ...
    lower(
        trim(json_extract_scalar(extra_data, "$.theater.allocine_movie_id"), " ")
    ) as theater_movie_id,
    lower(
    ...
    lower(coalesce(ean, trim(trim(json_extract(extra_data, "$.ean"), " "), '"'))) as ean,
...
from {{ ref("int_raw__offer") }} as offer
left join offer_product_data as ued on offer.offer_id = ued.offer_id
