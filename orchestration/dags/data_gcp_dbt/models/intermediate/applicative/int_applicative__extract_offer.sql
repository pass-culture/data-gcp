...
SELECT *,
LOWER(TRIM(JSON_EXTRACT_SCALAR(offer_extra_data, "$.author"))) AS author,
...
LOWER(TRIM(TRIM(JSON_EXTRACT(offer_extra_data, "$.isbn")," "),'"')) AS isbn,
LOWER(TRIM(TRIM(JSON_EXTRACT(offer_extra_data, "$.ean"), " "),'"')) AS ean,
LOWER(TRIM(TRIM(JSON_EXTRACT(offer_extra_data, "$.editeur")," "),'"')) AS book_editor,
LOWER(TRIM(TRIM(JSON_EXTRACT(offer_extra_data, "$.gtl_id")," "),'"')) AS titelive_gtl_id
