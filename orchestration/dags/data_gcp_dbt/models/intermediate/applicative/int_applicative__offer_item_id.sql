...
with
    items_grouping as (
        select
            ...
            case
                when (linkage_v2.item_id is not null and offer_product_id is null)
        ...
        left join
            {{ source("ml_linkage", "item_offer_mapping") }} linkage_v2
            on linkage_v2.offer_id = offer.offer_id
        qualify
            row_number() over (partition by offer_id order by offer_updated_date desc)
            = 1
    )

select offer_id, max(item_id) as item_id, max(new_item_id) as new_item_id
from items_grouping
