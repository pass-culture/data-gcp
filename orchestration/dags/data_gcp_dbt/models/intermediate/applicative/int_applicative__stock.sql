WITH bookings_grouped_by_stock AS (
    ...
    SELECT
        stock_id,
        SUM(CASE WHEN booking_is_cancelled = False THEN booking_quantity END) AS number_of_bookings,
        ...
        SUM(CASE WHEN NOT booking_is_cancelled THEN booking_intermediary_amount END) AS individual_theoretic_revenue,
        SUM(CASE WHEN booking_is_used THEN booking_intermediary_amount END) AS individual_real_revenue,
...
SELECT
    ...
    CASE WHEN s.stock_quantity IS NULL THEN NULL
        ELSE GREATEST( s.stock_quantity - COALESCE(bs.number_of_bookings, 0),0)
    END AS available_stock,
    number_of_bookings, -- on retire les cancelled? Avoir un seul calcul pour ce KPI
    bs.total_individual_bookings,
    bs.total_non_cancelled_individual_bookings,
    bs.total_used_individual_bookings,
    bs.individual_theoretic_revenue,
    bs.individual_real_revenue,
    ...
    AND (GREATEST(s.stock_quantity - COALESCE(bs.number_of_bookings, 0),0) > 0 OR s.stock_quantity IS NULL)
