...
select
    ...
    case
        when v.venue_activity = 'ART_GALLERY'
        then 'Galerie d’art'
        when v.venue_activity = 'ART_SCHOOL'
        then 'Conservatoire ou école d’arts'
        when v.venue_activity = 'ARTISTIC_COMPANY'
        then 'Compagnie artistique'
        when v.venue_activity = 'ARTS_CENTRE'
        then 'Centre d’arts'
        when v.venue_activity = 'ARTS_EDUCATION'
        then 'Formation ou enseignement artistique'
        when v.venue_activity = 'BOOKSTORE'
        then 'Librairie'
        when v.venue_activity = 'CINEMA'
        then 'Cinéma'
        when v.venue_activity = 'COMMUNITY_CENTRE'
        then 'Centre socio-culturel'
        when v.venue_activity = 'CREATIVE_ARTS_STORE'
        then 'Magasin d’arts créatifs'
        when v.venue_activity = 'CULTURAL_CENTRE'
        then 'Centre culturel pluridisciplinaire'
        when v.venue_activity = 'CULTURAL_MEDIATION'
        then 'Médiation culturelle'
        when v.venue_activity = 'DISTRIBUTION_STORE'
        then 'Magasin de distribution de produits culturels'
        when v.venue_activity = 'FESTIVAL'
        then 'Festival'
        when v.venue_activity = 'HERITAGE_SITE'
        then 'Site patrimonial, historique ou touristique'
        when v.venue_activity = 'LIBRARY'
        then 'Bibliothèque ou médiathèque'
        when v.venue_activity = 'MUSEUM'
        then 'Musée'
        when v.venue_activity = 'MUSIC_INSTRUMENT_STORE'
        then 'Magasin d’instruments de musique'
        when v.venue_activity = 'PERFORMANCE_HALL'
        then 'Salle de spectacles'
        when v.venue_activity = 'PRESS_OR_MEDIA'
        then 'Presse ou média'
        when v.venue_activity = 'PRODUCTION_OR_PROMOTION_COMPANY'
        then 'Société de production, tourneur ou label'
        when v.venue_activity = 'RECORD_STORE'
        then 'Disquaire'
        when v.venue_activity = 'SCIENCE_CENTRE'
        then 'Centre de culture scientifique, technique et industrielle'
        when v.venue_activity = 'STREAMING_PLATFORM'
        then 'Plateforme de streaming musique ou vidéo'
        when v.venue_activity = 'TOURIST_INFORMATION_CENTRE'
        then 'Office de tourisme'
        when v.venue_activity = 'TRAVELLING_CINEMA'
        then 'Cinéma itinérant'
        when v.venue_activity = 'GAMES_CENTRE'
        then 'Espace ludique'
        when v.venue_activity = 'TELEVISION_OR_VIDEO_STREAMING'
        then 'Télévision ou streaming vidéo'
        when v.venue_activity = 'RADIO_OR_MUSIC_STREAMING'
        then 'Radio ou streaming musical'
        when v.venue_activity = 'OTHER'
        then 'Autre'
        else v.venue_activity
    end as venue_activity,
    v.venue_label_id,
