with
    archive_folder_ref as (
        ...
        where concat(location, collection_id) like '/610%' and archived = false
    ),
    ...
    collections_w_root as (
        select
            *,
            concat(
                '/', split(concat(location, collection_id), '/')[safe_offset(1)]
            ) as root
    ...
    object_folder as (
        select
            ...
            slug,
            trim(replace(replace(slug, "___", '_'), '__', '_'), '_') as clean_slug,
            array_length(split(concat(location, collection_id), '/'))
            - 1 location_depth,
            case
                ...
                then concat(location, collection_id)
                    ...
                    concat(
                        ...
                        split(concat(location, collection_id), '/')[safe_offset(1)],
                        '/',
                        split(concat(location, collection_id), '/')[safe_offset(2)]
                    )
    ...
    slug_w_location_reduced as (
        select distinct
            ...
            trim(replace(replace(slug, "___", '_'), '__', '_'), '_') as clean_slug,
            concat(
                trim(replace(replace(slug, "___", '_'), '__', '_'), '_'), '_archive'
            ) as clean_slug_archive
        from collections_wo_perso
        ...
        where
            ...
            and personal_owner_id is null
            -- Filter the collections where the path depth does not exceed 3 elements
            -- to retrieve the level 2 parent folder of each card/dashboard.
            -- The archive folder has a maximum depth of 2 folders.
            -- Example: a card to be archived is located in the hierarchy
            -- /256/236/159/. Only the first two levels are retrieved
            -- to be able to move the card into the corresponding archive folder:
            -- /256/236_archive/159/.
    ...
    archive as (
        select
            ...
            case
                when
                    trim(replace(replace(slug, "___", '_'), '__', '_'), '_') in (
                        "1_externe_archive",
                        "2_interne_archive",
                        "3_operationnel_adhoc_archive",
                        "4_archive"
                    )
                then trim(replace(replace(slug, "___", '_'), '__', '_'), '_')
                else
                    concat(
                        trim(replace(replace(slug, "___", '_'), '__', '_'), '_'),
                        '_archive'
                    )
...
from object_folder
left join
    slug_w_location_reduced
    on object_folder.location_reduced_level_2 = slug_w_location_reduced.full_location
left join archive on archive.clean_slug = slug_w_location_reduced.clean_slug_archive
