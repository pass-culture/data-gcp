...
with
    base_data as (
        ...
        where
            ...
            and json_str is not null
            {% if is_incremental() %}
                and dbt_valid_from > (select max(this.last_updated_at) as dummy_alias from {{ this }} as this)
            {% endif %}
    ),
...
select
    ...
    {{ render_json_fields("with_product_type", "json_raw", cfg.specific_paper_fields_struct) }},
    -- Music-specific fields
    {{ render_json_fields("with_product_type", "json_raw", cfg.specific_music_fields_struct) }}
