with
    selected_users as (
        ...
        select
            ...
            0 as booking_cnt
        from {{ source("raw", "applicative_database_internal_user") }} ie
        left join
            {{ source("raw", "applicative_database_user") }} u on u.user_id = ie.user_id
    )
...
select
    ...
    coalesce(
        eu.total_theoretical_remaining_credit, eu.user_last_deposit_amount
