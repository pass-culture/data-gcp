...
group by ea.scholar_year, ey.educational_year_beginning_date, ey.educational_year_expiration_date, region_name, institution_academie
),
...
group by institution_region_name, institution_academy_name, scholar_year, partition_month
),
...
select
        ...
        sum(total_amount_spent_reimbursed) over (
            partition by institution_region_name, institution_academy_name, scholar_year
...
base_data as (
    ...
    left join booking_cumul_amount as cb
    ...
    where ia.educational_year_beginning_date <= cb.partition_month
...
{% for dim in dimensions %}
    ...
    select
        ...
        safe_divide(sum(cumulative_amount_spent),sum(total_institution_deposit_amount)) as kpi
    from base_data
    {% if is_incremental() %}
        where
            partition_month
    ...
    {% endif %}
    group by partition_month, scholar_year, updated_at, dimension_name, dimension_value
    union all
    select
        partition_month,
        scholar_year,
        ...
        safe_divide(sum(cumulative_amount_spent),sum(cumulative_bookings)) as kpi
    from base_data
    {% if is_incremental() %}
        where
            partition_month
