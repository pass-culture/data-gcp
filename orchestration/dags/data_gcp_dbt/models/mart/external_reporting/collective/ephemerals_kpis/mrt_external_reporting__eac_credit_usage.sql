...
{% set dimensions = [
    ...
    {"name": "REG", "value_expr": "institution_region_name", "value_filter": "venue_region_name"},
    {"name": "ACAD", "value_expr": "institution_academy_name", "value_filter": "venue_academy_name"},
] %}
...
all_months as (
    ...
    where gcb.collective_booking_status = 'REIMBURSED'
...
complete_grid as (
    ...
    inner join all_months as am
        on ad.scholar_year = am.scholar_year
),
...
complete_data as (
    select
        ...
        cg.partition_month,
        coalesce(md.total_amount_spent_reimbursed, 0) as total_amount_spent_reimbursed,
...
order by
    ...
    aggregation_by_scholar_year as (
        {% for dim in dimensions %}
            ...
            from base_data
            where {{ dim.value_filter }} = {{ dim.value_expr }}
            {% if is_incremental() %}
                and
                    partition_month
                    = date_trunc(date_sub(date("{{ ds() }}"), interval 1 month), month)
