...
with
    last_day as (
        ...
        from {{ ref("adage_involved_institution") }}
        where
            1 = 1
            {% if is_incremental() %}
...
{% for dim in dimensions %}
    ...
    select
        ...
        sum(involved.institutions) as numerator,
        sum(involved.total_institutions) as denominator,
        safe_divide(sum(involved.institutions), sum(involved.total_institutions)) as kpi
    from {{ ref("adage_involved_institution") }} as involved
