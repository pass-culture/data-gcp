...
select
    ...
    {% if social_network == 'instagram' %}instagram_publication_media_type{% else %}null as instagram_publication_media_type {% endif %} ,
    ...
    {% if social_network == 'instagram' %}instagram_{{ field }}{% else %}null{% endif %} as instagram_{{ field }},
    {% endfor %}

    -- Tiktok specific metrics
    {% for field in tiktok_specific_fields %}
    {% if social_network == 'tiktok' %}tiktok_{{ field }}{% else %}null{% endif %} as tiktok_{{ field }}
    {%- if not loop.last %},{% endif %}
    {% endfor %}

from {{ ref("int_" ~ social_network ~ "__publication_detail") }}
where publication_export_date = (
    select max(social_network_publication_detail.publication_export_date) as max_export_date
    from {{ ref("int_" ~ social_network ~ "__publication_detail") }} as social_network_publication_detail
)
