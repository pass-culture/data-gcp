with
    ...
    bookings_info as (
        select
            ...
            user.user_id,
            sum(bookings.diversity_score) as total_diversity,
            count(
        ...
        where not bookings.booking_is_cancelled
        group by user.deposit_expiration_date, user.user_id
    ),
...
select
    ...
    coalesce(sum(b.total_diversity), 0) as total_diversity_score,
