...
{% endif %}
        select
            ...
            safe_divide(sum(total_users_last_12_months),sum(total_population_last_12_months)) as kpi
        from
            {{ ref('mrt_native__monthly_beneficiary_coverage')}}
        where
            population_decimal_age = "19"
            and population_snapshot_month <= date_trunc(current_date, month)
            {% if is_incremental() %}
            and date_trunc(population_snapshot_month, month) = date_trunc(date("{{ ds() }}"), month)
            {% endif %}
