{% test snapshot_volume_anomalies(model, remove_outliers=false, date_column=None, check_date=None, days_back=30, sensitivity=3, nr_sensitivity=None, dr_sensitivity=None) %}
...
{% if target_date %}
    {% set target_ts = "timestamp('" ~ target_date ~ "')" %}
{% else %}
    {% set target_ts = "timestamp(current_date())" %}
{% endif %}

{# 3. Calculate Boundaries in SQL #}
{% set lower_bound = "timestamp_sub(" ~ target_ts ~ ", interval " ~ days_back ~ " day)" %}
...
{% set nr_sensitivity = nr_sensitivity if nr_sensitivity is not none else sensitivity %}
{% set dr_sensitivity = dr_sensitivity if dr_sensitivity is not none else sensitivity %}


with raw_window_data as (
    select
        dbt_valid_from,
        dbt_valid_to
    from {{ model }}
    where 
      -- 1. PARTITION PRUNING: Only scan Active (NULL) or recently closed partitions
      (dbt_valid_to >= {{ lower_bound }} OR dbt_valid_to IS NULL)
      -- 2. LOGICAL FILTER: Row must overlap with the window
      and (dbt_valid_from >= {{ lower_bound }} OR dbt_valid_to >= {{ lower_bound }})
),
...
daily_events as (
    ...
    where dbt_valid_from >= {{ lower_bound }}
...
-- PASS 1: Initial stats from ALL historical data (strictly LESS THAN target date)
initial_stats as (
    select
...
training_set as (
    ...
    where test_set.metric_date < date({{ target_ts }})  -- Only historical baseline data
    {% if remove_outliers %}
      -- Remove historical outliers using the general sensitivity threshold
      -- This creates a cleaner baseline for comparison
      and abs(test_set.rows_added - stats.avg_added) <= ({{ sensitivity }} * greatest(stats.std_added, 1))
      and abs(test_set.rows_removed - stats.avg_removed) <= ({{ sensitivity }} * greatest(stats.std_removed, 1))
    {% endif %}
),
...
final_analysis as (
    select
        ...
        abs((test_set.rows_added - stats.avg_added) / greatest(stats.std_added, 1)) as z_score_added,
        abs((test_set.rows_removed - stats.avg_removed) / greatest(stats.std_removed, 1)) as z_score_removed
...
select
    *,
    case
        when z_score_added > {{ sensitivity }} and z_score_removed > {{ sensitivity }} then 'Anomaly: High Volume of UPDATES'
        when z_score_added > {{ nr_sensitivity }} then 'Anomaly: Spike in CREATED records'
        when z_score_removed > {{ dr_sensitivity }} then 'Anomaly: Spike in DELETED records'
        else 'Normal'
    end as anomaly_type
from final_analysis
where 
