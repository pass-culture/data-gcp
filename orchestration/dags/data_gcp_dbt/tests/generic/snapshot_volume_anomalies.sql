{% test snapshot_volume_anomalies(
    ...
    - enabled: Enable or disable the test (default: true). When disabled, the test returns 0 rows and is marked as PASS (for CI concerns).
    #}
    {% set is_ci = target.profile_name == "CI" or env_var("IS_CI", "false") == "true" %}

    {% if not enabled or is_ci %}
        {# EXIT EARLY: This query returns nothing and requires NO tables to exist #}
    ...
    {% else %}
        ...
        {% else %}
            ...
            with
                -- Identify IDs that existed BEFORE the window to disambiguate Updates
                -- vs
                -- Inserts
                ...
                raw_window_data as (
                    ...
                    from {{ model }}
                    where
                        -- 1. PARTITION PRUNING: Only scan Active (NULL) or recently
                        -- closed
                        -- partitions
                    ...
                    select
                        ...
                        count(
                            distinct case
                                when
                                    was_created
                                    and (was_closed or was_previously_known = 1)
                                then id
                            end
                        ) as records_updated,
                        count(
                        ...
                        count(
                            distinct case
                                when was_closed and not was_created then id
                            end
                        ) as records_deleted
                ...
                -- OPTIONAL PASS: Clean the history by removing outliers from BASELINE
                -- ONLY
                training_set as (
                    select
                ...
                *,
                case
                    ...
                    when
                        ...
                        and z_score_deletes < -{{ dr_sensitivity }}
                    then
                        'Anomaly: Drop in INSERTS and DELETES (possible pipeline issue)'
            ...
            from final_analysis
            where
                -- Detect anomalies using the specific thresholds (both spikes AND
                -- drops)
