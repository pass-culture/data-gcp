...
def get_bookings(start_date, end_date):
    query = f"""
        select user_id, 
        (CASE WHEN offer.offer_type = 'ThingType.LIVRE_EDITION' THEN CONCAT('product-', offer.offer_product_id) ELSE CONCAT('offer-', offer.offer_id) END) AS offer_id,
...
def main():
    ...
    # TODO replace with datetime formatting
    END_DATE = "2021-04-13"
