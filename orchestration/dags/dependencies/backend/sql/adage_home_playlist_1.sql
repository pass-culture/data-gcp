-- Les lieux permanents ayant des offres vitrines actives
WITH offerer_offer_info AS (
  ...
  FROM `{{ bigquery_analytics_dataset }}`.enriched_collective_offer_data o
	JOIN `{{ bigquery_analytics_dataset }}`.applicative_database_venue v ON v.venue_id=o.venue_id
	AND venue_is_permanent IS TRUE 
  WHERE offer_is_template IS TRUE AND collective_offer_is_active
),
