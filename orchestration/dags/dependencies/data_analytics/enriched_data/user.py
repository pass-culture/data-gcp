    ...
    return f"""
        CREATE OR REPLACE TABLE {dataset}.enriched_user_data AS (
            SELECT
                ...
                CASE WHEN TIMESTAMP(deposit.expirationDate) < CURRENT_TIMESTAMP() OR actual_amount_spent >= deposit.amount THEN TRUE ELSE FALSE END AS user_is_former_beneficiary,
                CASE WHEN (TIMESTAMP(deposit.expirationDate) >= CURRENT_TIMESTAMP() AND actual_amount_spent < deposit.amount) AND user_is_active THEN TRUE ELSE FALSE END AS user_is_current_beneficiary
            FROM {dataset}.{table_prefix}user AS user
            ...
            LEFT JOIN deposit ON user.user_id = deposit.userId
            WHERE user.user_is_beneficiary
        );
    """
