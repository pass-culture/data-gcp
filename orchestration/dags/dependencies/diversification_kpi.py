...
def get_diversification_feature(list_used, feature_value):
    div=0
    if feature_value not in list_used :
        div=1
    return div
...
def calculate_diversification_per_feature(df_clean, features):
    ...
    for i in range(0, len(df_clean)):
        ...
        for feature in features:
            feature_value = df_clean.iloc[i][feature]
            # Use a unique venue for numerical bookings
            if (feature == 'venue' and df_clean.iloc[i]['format'] == 'numerical'):
                feature_value = 'numerical'
            multiplicator = 1

            # Calculate diversification for feature
            if df_clean.iloc[i - 1].user_id != df_clean.iloc[i].user_id or i == 0:
                list_per_feature[feature] = []
                list_per_feature_free[feature] = []
                div = 0
                qpi_point_given = False
            elif (feature == 'type' and df_clean.iloc[i]['subcategory'] == 'SEANCE_CINE' and feature_value not in [
                np.nan]):
                div_temp = 0
                for theme in feature_value:
                    div_temp = div_temp + get_diversification_feature(list_per_feature[feature], theme)
                div = (1 if div_temp > 0 else 0)
            else:
                div = get_diversification_feature(list_per_feature[feature], feature_value)

            # Multiplicator of the diversification
            if (feature == 'venue' and df_clean.iloc[i]['category'] != 'LIVRE'):
                multiplicator = multiplicator * 2
            if (df_clean.iloc[i]['booking_amount'] == 0 and feature_value in list_per_feature_free[feature]):
                multiplicator = multiplicator * 0
            else:
                if (feature_value in list_per_feature_free[feature]):
                    multiplicator = multiplicator * 0.5
                if (df_clean.iloc[i]['booking_amount'] == 0):
                    multiplicator = multiplicator * 0.5
                    list_per_feature_free[feature] = add_feature_if_feature_not_booked(list_per_feature_free[feature],
                                                                                       feature_value)

            # Add feature value to the list of values booked
            if (df_clean.iloc[i]['booking_amount'] != 0):
                if (feature == 'type' and df_clean.iloc[i]['subcategory'] == 'SEANCE_CINE' and feature_value not in [
                    np.nan]):
                    for theme in feature_value:
                        list_per_feature[feature] = add_feature_if_feature_not_booked(list_per_feature[feature], theme)
                else:
                    list_per_feature[feature] = add_feature_if_feature_not_booked(list_per_feature[feature],
                                                                                  feature_value)
        ...
        if (qpi_point_given == False):
            subcat = df_clean.iloc[i]['subcategory']
            if (subcat in df_clean.columns):
                if (df_clean.iloc[i][subcat] == False):
