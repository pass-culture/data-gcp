def define_table_venue_locations(dataset, table_prefix=""):
    return f"""
     with venue_epci AS (
            ...
            epci_code,
            epci_name
            FROM `{dataset}.{table_prefix}venue` venue
            JOIN (SELECT 
                        epci.epci_code,
                        epci.epci_name, 
                        geo_shape
                        FROM `{dataset}.epci` epci) c on ST_CONTAINS(c.geo_shape, ST_GEOGPOINT(venue.venue_longitude,venue.venue_latitude))),
            ...
            qpv_communes
            FROM `{dataset}.{table_prefix}venue` venue 
             JOIN (SELECT code_quartier as code_qpv,
                                    noms_des_communes_concernees as qpv_name,
                                    commune_qp as qpv_communes,
                                    geoshape
                                FROM `{dataset}.QPV` )b ON ST_CONTAINS(b.geoshape, ST_GEOGPOINT(venue.venue_longitude,venue.venue_latitude))),
            ...
            FROM `{dataset}.{table_prefix}venue` venue 
             JOIN (SELECT 
                        ...
                        ZRR.Code_Postal, 
                        ZRR.geo_shape_insee
                        FROM `{dataset}.ZRR` ZRR) d on ST_CONTAINS(d.geo_shape_insee, ST_GEOGPOINT(venue.venue_longitude,venue.venue_latitude)))
