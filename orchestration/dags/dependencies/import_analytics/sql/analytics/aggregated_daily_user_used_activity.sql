...
user_active_dates AS (
    ...
    FROM
        ...
        JOIN days ON days.day BETWEEN date(`{{ bigquery_analytics_dataset }}.enriched_deposit_data`.deposit_creation_date)
        AND date(`{{ bigquery_analytics_dataset }}.enriched_deposit_data`.deposit_expiration_date)
),
...
diversification as (
  ...
  FROM `{{ bigquery_analytics_dataset }}.diversification_booking` divers
  LEFT JOIN `{{ bigquery_analytics_dataset }}.enriched_booking_data` book
