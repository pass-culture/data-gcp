WITH bookings_per_stock AS (
    ...
    FROM
        `{{ bigquery_clean_dataset }}`.applicative_database_booking_history AS booking
...
FROM
    `{{ bigquery_clean_dataset }}`.applicative_database_stock_history AS stock
    JOIN `{{ bigquery_clean_dataset }}`.applicative_database_offer_history AS offer ON stock.offer_id = offer.offer_id
