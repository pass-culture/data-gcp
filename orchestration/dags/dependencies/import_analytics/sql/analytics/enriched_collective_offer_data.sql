...
SELECT
    ...
    region_department.academy_name AS venue_academie,
...
FROM
    ...
    LEFT JOIN `{{ bigquery_analytics_dataset }}`.region_department ON region_department.num_dep = venue.venue_department_code
    LEFT JOIN `{{ bigquery_analytics_dataset }}`.region_department venue_region ON venue_region.num_dep = venue.venue_department_code
