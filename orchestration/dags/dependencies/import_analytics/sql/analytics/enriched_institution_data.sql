WITH ranked_deposit AS (
    ...
    FROM
        `{{ bigquery_analytics_dataset }}`.applicative_database_educational_deposit AS educational_deposit
        JOIN `{{ bigquery_analytics_dataset }}`.applicative_database_educational_year AS educational_year ON educational_deposit.educational_year_id = educational_year.adage_id
),
...
last_booking AS (
    ...
    FROM
        bookings_infos
        JOIN `{{ bigquery_analytics_dataset }}`.applicative_database_collective_stock AS collective_stock ON bookings_infos.collective_stock_id = collective_stock.collective_stock_id
...
students_educonnectes AS (
    ...
    FROM
        `{{ bigquery_analytics_dataset }}`.applicative_database_beneficiary_fraud_check AS beneficiary_fraud_check
        LEFT JOIN `{{ bigquery_analytics_dataset }}`.enriched_user_data AS enriched_user_data ON beneficiary_fraud_check.user_id = enriched_user_data.user_id
    WHERE
        type = 'EDUCONNECT'
...
FROM
    ...
    LEFT JOIN `{{ bigquery_analytics_dataset }}`.academie_dept AS academie_dept ON educational_institution.institution_departement_code = academie_dept.code_dpt
