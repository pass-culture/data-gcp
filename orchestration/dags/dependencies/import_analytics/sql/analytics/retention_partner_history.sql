...
FROM
  `{{ bigquery_analytics_dataset }}`.enriched_cultural_partner_data_v2
CROSS JOIN
  UNNEST(GENERATE_ARRAY(0, DATE_DIFF(CURRENT_DATE(), '2022-07-01', DAY))) AS offset
WHERE
  DATE_ADD(DATE('2022-07-01'), INTERVAL offset DAY) >= first_offer_creation_date -- Les jours depuis la 1ère offre
AND DATE_ADD(DATE('2022-07-01'), INTERVAL offset DAY) < CURRENT_DATE() -- Que des jours avant aujourd'hui
)

, all_days_and_bookability AS (
SELECT
    all_activated_partners_and_days_since_activation.venue_id
    , first_offer_creation_date
    , day
    ,COALESCE(total_bookable_offers,0) AS total_bookable_offers
FROM all_activated_partners_and_days_since_activation
LEFT JOIN `{{ bigquery_analytics_dataset }}`.bookable_partner_history ON bookable_partner_history.partner_id = all_activated_partners_and_days_since_activation.venue_id
