WITH bookings_per_stock AS (
    ...
    FROM
        `{{ bigquery_raw_dataset }}`.applicative_database_collective_booking AS collective_booking
    GROUP BY
        collective_stock_id
)
...
FROM `{{ bigquery_raw_dataset }}`.applicative_database_collective_stock AS collective_stock
JOIN `{{ bigquery_raw_dataset }}`.applicative_database_collective_offer AS collective_offer
...
SELECT
    ...
    ,venue.venue_managing_offerer_id AS offerer_id
FROM
    `{{ bigquery_raw_dataset }}`.applicative_database_collective_offer_template AS template
    JOIN `{{ bigquery_raw_dataset }}`.applicative_database_venue AS venue ON venue.venue_id = template.venue_id