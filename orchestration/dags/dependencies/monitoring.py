...
def _define_recommendation_booking_funnel(start_date, end_date):
    return f"""
        WITH booking_events AS (
            ...
            MAX(CASE WHEN params.key = "offerId" THEN SAFE_CAST(params.value.double_value AS INT64) ELSE NULL END) AS double_offer_id,
            MAX(CASE WHEN params.key = "offerId" THEN SAFE_CAST(params.value.string_value AS INT64) ELSE NULL END) AS string_offer_id,
