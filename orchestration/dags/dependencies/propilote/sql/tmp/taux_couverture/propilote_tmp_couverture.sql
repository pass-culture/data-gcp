with
    coverage_19 as (
        ...
        where population_decimal_age = 19 and active_month <= date_trunc(current_date, month)
        group by 1, 2, 3, 4, 5
    ),
    ...
    coverage_18 as (
        select
            ...
            sum(population_last_12_months) as denominator
        from `{{ bigquery_analytics_dataset }}.monthly_beneficiary_coverage` as up
        where population_decimal_age = 18 and active_month <= date_trunc(current_date, month)
        group by 1, 2, 3, 4, 5
    ),
    ...
    coverage_17 as (
        select
            ...
            sum(population_last_12_months) as denominator
        from `{{ bigquery_analytics_dataset }}.monthly_beneficiary_coverage` as up
        where population_decimal_age = 17 and active_month <= date_trunc(current_date, month)
        group by 1, 2, 3, 4, 5
    ),
    ...
    coverage_16 as (
        select
            ...
            sum(population_last_12_months) as denominator
        from `{{ bigquery_analytics_dataset }}.monthly_beneficiary_coverage` as up
        where population_decimal_age = 16 and active_month <= date_trunc(current_date, month)
        group by 1, 2, 3, 4, 5
    )
