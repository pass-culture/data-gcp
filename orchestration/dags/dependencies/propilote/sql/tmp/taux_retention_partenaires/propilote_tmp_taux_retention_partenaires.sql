...
all_partners AS (
  ...
  LEFT JOIN `{{ bigquery_analytics_dataset }}.global_cultural_partner` global_cultural_partner ON retention_partner_history.partner_id  = mrt_global__cultural_partner.partner_id
  LEFT JOIN `{{ bigquery_analytics_dataset }}.region_department` region_department ON mrt_global__cultural_partner.partner_department_code = region_department.num_dep
  GROUP BY
    1,
    2,
    3
