...
-- *** Missing utm
with
    sendinblue_transactional as (
        select distinct * from `{{ bigquery_clean_dataset }}.sendinblue_transactional`
    ),
...
left join user_traffic on sendinblue_transactional.tag = user_traffic.traffic_campaign
