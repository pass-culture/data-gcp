...
INNER JOIN `{{ bigquery_int_applicative_dataset }}`.`offer_item_id` int_applicative__offer_item_id
        ON int_applicative__offer_item_id.offer_id = offer.offer_id
INNER JOIN `{{ bigquery_ml_preproc_dataset }}`.`item_embedding_reduced_16` item_embedding_reduced
        ON int_applicative__offer_item_id.item_id = item_embedding_reduced.item_id
GROUP BY 1,2,3,4,5
)
