...
with DAG(
    ...
    for cluster_config in CLUSTERING_CONFIG:
        ...
        gce_instance = f"clusterisation-{job_name}{VERSION_SUFFIX.replace('_', '-')}-{ENV_SHORT_NAME}"
        ...
        preprocess_clustering = SSHGCEOperator(
            ...
            f"--input-table-name item_embedding_clusterisation{VERSION_SUFFIX} "
            f"--output-dataset-name tmp_{ENV_SHORT_NAME} "
            f"--output-table-name {DATE}_{cluster_prefix}_import_item_clusters_preprocess{VERSION_SUFFIX} "
        ...
        generate_clustering = SSHGCEOperator(
            ...
            f"--input-table-name {DATE}_{cluster_prefix}_import_item_clusters_preprocess{VERSION_SUFFIX} "
            f"--output-dataset-name ml_preproc_{ENV_SHORT_NAME} "
            f"--output-table-name {cluster_prefix}_item_cluster{VERSION_SUFFIX} "
            f"--config-file-name {cluster_config_file_name} ",
            deferrable=True,
        )
