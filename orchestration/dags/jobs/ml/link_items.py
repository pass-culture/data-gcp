...
with DAG(
    ...
    import_sources = BigQueryExecuteQueryOperator(
        ...
        destination_dataset_table=f"{BIGQUERY_TMP_DATASET}.{DATE}_{dag_config['input_sources_table_name']}",
        dag=dag,
    )
    ...
    import_candidates = BigQueryExecuteQueryOperator(
        ...
        write_disposition="WRITE_TRUNCATE",
        use_legacy_sql=False,
        destination_dataset_table=f"{BIGQUERY_TMP_DATASET}.{DATE}_{dag_config['input_candidates_table_name']}",
        dag=dag,
    )
        ...
        configuration={
            "extract": {
                "sourceTable": {
                    ...
                    "tableId": f"{DATE}_{dag_config['input_sources_table_name']}",
                },
                "compression": None,
                "destinationUris": f"{dag_config['STORAGE_PATH']}/{dag_config['input_sources_table_name']}/data-*.parquet",
        ...
        configuration={
            "extract": {
                "sourceTable": {
                    "projectId": GCP_PROJECT_ID,
                    "datasetId": BIGQUERY_TMP_DATASET,
                    "tableId": f"{DATE}_{dag_config['input_candidates_table_name']}",
                },
                "compression": None,
                "destinationUris": f"{dag_config['STORAGE_PATH']}/{dag_config['input_candidates_table_name']}/data-*.parquet",
                "destinationFormat": "PARQUET",
            }
        },
        dag=dag,
    ...
    build_linkage_vector = SSHGCEOperator(
        ...
        f"--input-table-name {dag_config['input_sources_table_name']} ",
        dag=dag,
    )
    ...
    get_linkage_candidates = SSHGCEOperator(
        ...
        f"--source-gcs-path {dag_config['STORAGE_PATH']} "
        f"--input-table-name {dag_config['input_candidates_table_name']} "
