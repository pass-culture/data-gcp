...
dag_config = {
    "STORAGE_PATH": f"gs://{MLFLOW_BUCKET_NAME}/linkage_vector_{ENV_SHORT_NAME}/name_{DATE}/{DATE}_item_embbedding_data",
...
gce_params = {
    "instance_name": f"linkage-semantic-vector-{ENV_SHORT_NAME}",
    "instance_type": {
        "dev": "n1-standard-2",
        "stg": "n1-standard-8",
        "prod": "n1-standard-16",
...
with DAG(
    ...
    export_task = BigQueryExecuteQueryOperator(
        ...
        sql=(IMPORT_TRAINING_SQL_PATH / f"linkage_semantic_vector.sql").as_posix(),
    ...
    containerize_semanctic_vector = SSHGCEOperator(
        task_id="containerize_linkage_semantic_vector",
        instance_name="{{ params.instance_name }}",
        base_dir=dag_config["BASE_DIR"],
        command="python deploy_semantic_name.py "
