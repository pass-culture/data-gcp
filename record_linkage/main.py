...
def _setup_matching(c_cl,featdict):
    for feature in featdict.keys():
        ...
        if feature_dict["method"]=="exact":
            ...
            c_cl.string(feature, feature, method=method, threshold=threshold, label=feature)
    return c_cl
...
def main():
    ...
    subcat_wo_performer=[x for x in subcat_all if x not in SUBCATEGORIES_WITH_PERFORMER]
    df_to_link_performer=df_offers_to_link_clean.query(f"""offer_subcategoryId in {tuple(SUBCATEGORIES_WITH_PERFORMER)} """)
    df_to_link_non_performer=df_offers_to_link_clean.query(f"""offer_subcategoryId in {tuple(subcat_wo_performer)} """)
    ...
    for grp_smp in data_and_hyperparams_dict.keys():
        df_offers_matched_list.append(get_matched_df(data_and_hyperparams_dict[grp_smp]))
