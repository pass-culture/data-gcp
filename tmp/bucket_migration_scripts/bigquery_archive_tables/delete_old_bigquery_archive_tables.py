#!/usr/bin/env python3
"""
Delete Old BigQuery Archive Tables Folder Script

This script safely deletes the /bigquery_archive_tables folder and all its contents from the
old data-bucket-{env} after confirming that data has been successfully migrated
to the new legacy-archive-bucket-{env} bucket.
...
class OldBigQueryArchiveTablesFolderDeleter:
    """Safely handles deletion of the /bigquery_archive_tables folder from old bucket structure."""

    def __init__(self, env: str, dry_run: bool = True):
        self.env = env
        self.dry_run = dry_run
        self.old_bucket = f"data-bucket-{env}"
        self.new_bucket = f"legacy-archive-bucket-{env}"
        self.old_folder_path = f"gs://{self.old_bucket}/bigquery_archive_tables/"
        self.new_archive_path = f"gs://{self.new_bucket}/archive/"

        # Configure logging
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        log_filename = f"delete_old_bigquery_archive_tables_{env}_{timestamp}.log"
        ...
        self.manifest_file = (
            f"old_bigquery_archive_tables_manifest_{env}_{timestamp}.json"
        )
    ...
    def list_bigquery_archive_tables_contents(self) -> List[Dict[str, str]]:
        """List all contents in the /bigquery_archive_tables folder with metadata."""
        self.logger.info(f"Scanning contents of {self.old_folder_path}")
        ...
        try:
            ...
            if result.returncode != 0:
                if (
                    ...
                    self.logger.info(
                        "BigQuery archive tables folder does not exist or is already empty"
                    )
                    return []
                else:
                    self.logger.error(
                        f"Error listing bigquery_archive_tables contents: {result.stderr}"
                    )
                    return []
            ...
            self.logger.error(f"Error scanning bigquery_archive_tables folder: {e}")
            return []
    ...
    def verify_new_bucket_has_data(self) -> bool:
        """Verify that the new legacy archive bucket contains migrated data."""
        try:
            result = subprocess.run(
                ["gcloud", "storage", "ls", "--recursive", self.new_archive_path],
                capture_output=True,
                text=True,
                check=False,
            )
            ...
            if result.returncode == 0:
                ...
                if new_files:
                    self.logger.info(
                        f"Verified new legacy archive bucket has {len(new_files)} files"
                    )
                    return True
                else:
                    self.logger.warning(
                        "New legacy archive bucket exists but appears empty"
                    )
                    return False
            else:
                self.logger.warning(
                    f"Cannot access new legacy archive bucket: {self.new_archive_path}"
                )
                return False
    ...
    def delete_old_bigquery_archive_tables_folder(self) -> bool:
        """Delete the entire /bigquery_archive_tables folder from old bucket."""
        if self.dry_run:
            self.logger.info(
                f"DRY RUN - Would execute: gcloud storage rm --recursive {self.old_folder_path}"
            )
            return True

        try:
            self.logger.info(f"EXECUTING DELETION: {self.old_folder_path}")

            result = subprocess.run(
                ["gcloud", "storage", "rm", "--recursive", self.old_folder_path],
                capture_output=True,
                text=True,
                check=False,
            )
            ...
            if result.returncode == 0:
                self.logger.info(
                    "Successfully deleted old /bigquery_archive_tables folder"
                )
                if result.stdout:
                ...
                self.logger.error(
                    "Failed to delete old /bigquery_archive_tables folder"
                )
    ...
    def run_deletion(self) -> bool:
        ...
        if not self.check_bucket_exists(self.new_bucket):
            self.logger.error(
                f"New legacy archive bucket {self.new_bucket} does not exist!"
            )
            return False

        if not self.verify_new_bucket_has_data():
            self.logger.error(
                "New legacy archive bucket appears to be empty. Migration may not be complete."
            )
            if not self.dry_run:
        ...
        # List all files in old bigquery_archive_tables folder
        files = self.list_bigquery_archive_tables_contents()

        if not files:
            self.logger.info(
                "No files found in old /bigquery_archive_tables folder. Nothing to delete."
            )
            return True
        ...
        if not self.dry_run:
            ...
            self.logger.warning(f"From: {self.old_folder_path}")

            print("\nType the following EXACTLY to confirm deletion:")
            confirmation_text = f"DELETE {self.env} OLD BIGQUERY ARCHIVE TABLES"
        ...
        # Perform deletion
        success = self.delete_old_bigquery_archive_tables_folder()
...
def main():
    parser = argparse.ArgumentParser(
        description="Delete old /bigquery_archive_tables folder from data-bucket after migration",
...
- Dry run mode by default
- Creates backup manifest before deletion
- Verifies new legacy archive bucket has data
...
- Comprehensive logging
    ...
    if not dry_run:
        ...
        print(f"Path: gs://data-bucket-{args.env}/bigquery_archive_tables/")
        print()
    ...
    deleter = OldBigQueryArchiveTablesFolderDeleter(args.env, dry_run)
