#!/usr/bin/env python3
"""
BigQuery Archive Tables Migration Script

This script migrates all bigquery_archive_tables data from the old bucket structure to the new
dedicated legacy archive bucket.

Migration path:
- From: gs://data-bucket-{env}/bigquery_archive_tables/
- To: gs://de-bigquery-data-archive-{env}/archive/
...
class BigQueryArchiveTablesMigrator:
    """Handles migration of bigquery_archive_tables data from old to new bucket structure."""
    ...
    def __init__(self, env: str, dry_run: bool = True):
        ...
        log_filename = f"migrate_bigquery_archive_tables_{env}_{timestamp}.log"
    ...
    def list_bigquery_archive_tables_contents(self) -> List[str]:
        """List all contents in the /bigquery_archive_tables folder."""
        ...
        try:
            ...
            if result.returncode != 0:
                if (
                    ...
                    self.logger.info(
                        "BigQuery archive tables folder does not exist or is empty"
                    )
                    return []
                else:
                    self.logger.error(
    ...
    def migrate_bigquery_archive_tables_data(self) -> Tuple[bool, int]:
        """
        Migrate all bigquery_archive_tables data using gcloud storage rsync.
        ...
        if not files:
            self.logger.info("No bigquery_archive_tables data to migrate")
            return True, 0
        ...
        if self.dry_run:
            self.logger.info(
                "DRY RUN - BigQuery archive tables data that would be migrated:"
            )
        ...
        try:
            ...
            if result.returncode == 0:
                self.logger.info("Successfully migrated bigquery_archive_tables data")
                if result.stdout:
                    self.logger.info(f"gcloud storage output: {result.stdout}")
                return True, len(files)
            else:
                self.logger.error("BigQuery archive tables data migration failed")
    ...
    def run_migration(self) -> bool:
        ...
        # Perform migration
        success, files_migrated = self.migrate_bigquery_archive_tables_data()
...
def main():
    parser = argparse.ArgumentParser(
        description="Migrate bigquery_archive_tables data to new bucket structure"
    )
    parser.add_argument(
        "--env",
        required=True,
    ...
    if not dry_run:
        response = input(
            f"Are you sure you want to migrate bigquery_archive_tables data for {args.env}? (yes/no): "
        )
        if response.lower() != "yes":
            print("Migration cancelled.")
            return 1

    migrator = BigQueryArchiveTablesMigrator(args.env, dry_run)
