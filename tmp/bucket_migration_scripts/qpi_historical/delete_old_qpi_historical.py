#!/usr/bin/env python3
"""
Delete Old QPI Historical Folder Script

This script safely deletes the /QPI_historical folder and all its contents from the
old data-bucket-{env} after confirming that data has been successfully migrated
to the new seed-data-bucket-{env} bucket.
...
class OldQPIHistoricalFolderDeleter:
    """Safely handles deletion of the /QPI_historical folder from old bucket structure."""
    ...
    def __init__(self, env: str, dry_run: bool = True):
        ...
        self.old_folder_path = f"gs://{self.old_bucket}/QPI_historical/"
        self.new_historical_path = f"gs://{self.new_bucket}/historical/"

        # Configure logging
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        log_filename = f"delete_old_qpi_historical_{env}_{timestamp}.log"
        ...
        self.manifest_file = f"old_qpi_historical_manifest_{env}_{timestamp}.json"
    ...
    def list_qpi_historical_contents(self) -> List[Dict[str, str]]:
        """List all contents in the /QPI_historical folder with metadata."""
        ...
        try:
            ...
            if result.returncode != 0:
                if (
                    ...
                    self.logger.info(
                        "QPI_historical folder does not exist or is already empty"
                    )
                    return []
                else:
                    self.logger.error(
    ...
    def create_manifest(self, files: List[Dict[str, str]]) -> bool:
        ...
        manifest = {
            ...
            "new_path": self.new_historical_path,
    ...
    def verify_new_bucket_has_data(self) -> bool:
        """Verify that the new seed data bucket contains migrated data."""
        try:
            result = subprocess.run(
                ["gcloud", "storage", "ls", "--recursive", self.new_historical_path],
                capture_output=True,
                text=True,
                check=False,
            )
            ...
            if result.returncode == 0:
                ...
                if new_files:
                    self.logger.info(
                        f"Verified new seed data bucket has {len(new_files)} files"
                    )
                    return True
                else:
                    self.logger.warning("New seed data bucket exists but appears empty")
                    return False
            else:
                self.logger.warning(
                    f"Cannot access new seed data bucket: {self.new_historical_path}"
                )
                return False
    ...
    def delete_old_qpi_historical_folder(self) -> bool:
        """Delete the entire /QPI_historical folder from old bucket."""
        if self.dry_run:
            self.logger.info(
        ...
        try:
            ...
            if result.returncode == 0:
                self.logger.info("Successfully deleted old /QPI_historical folder")
                if result.stdout:
                    self.logger.info(f"gcloud storage output: {result.stdout}")
                return True
            else:
                self.logger.error("Failed to delete old /QPI_historical folder")
    ...
    def run_deletion(self) -> bool:
        ...
        if not self.check_bucket_exists(self.new_bucket):
            self.logger.error(f"New seed data bucket {self.new_bucket} does not exist!")
            return False

        if not self.verify_new_bucket_has_data():
            self.logger.error(
                "New seed data bucket appears to be empty. Migration may not be complete."
            )
            if not self.dry_run:
        ...
        # List all files in old QPI_historical folder
        files = self.list_qpi_historical_contents()

        if not files:
            self.logger.info(
                "No files found in old /QPI_historical folder. Nothing to delete."
            )
            return True
        ...
        if not self.dry_run:
            ...
            confirmation_text = f"DELETE {self.env} OLD QPI HISTORICAL"
        ...
        # Perform deletion
        success = self.delete_old_qpi_historical_folder()
...
def main():
    parser = argparse.ArgumentParser(
        description="Delete old /QPI_historical folder from data-bucket after migration",
...
- Dry run mode by default
- Creates backup manifest before deletion
- Verifies new seed data bucket has data
...
- Comprehensive logging
    ...
    if not dry_run:
        ...
        print(f"Path: gs://data-bucket-{args.env}/QPI_historical/")
        print()
    ...
    deleter = OldQPIHistoricalFolderDeleter(args.env, dry_run)
